...
bool native_syscall_fuzzer = FALSE;
...
static void usage(void)
{
	...
	outputerr(" --native: Issue native syscall fuzzing.\n");
	outputerr("\n");
...
void parse_args(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, paramstr, longopts, &opt_index)) != -1) {
		...
		case 0:
			if (strcmp("native", longopts[opt_index].name) == 0)
				native_syscall_fuzzer = TRUE;
