...
int main(int argc, char* argv[])
{
    ...
    const long MYST_TCALL_GET_SYSCALL_FUZZER_PAYLOAD = 2101;
    long syscall_payload_ptr = 0;
	if (!native_syscall_fuzzer)
	{
		if (!syscall(MYST_TCALL_GET_SYSCALL_FUZZER_PAYLOAD, &syscall_payload_ptr))
		{
			outputstd(">>>>>>>>>>>>>> MYST_TCALL_GET_SYSCALL_FUZZER_PAYLOAD >>>>>>>>>>>>>>>>> \n");
